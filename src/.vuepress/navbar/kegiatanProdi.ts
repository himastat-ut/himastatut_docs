export default {
    text: "Prodi Statistika", 
    icon: "brain",
    link: "/archived/",
    children: [
      {
        text: "Apa itu Program Studi Statistika, Universitas Terbuka?",
        icon: "file-zipper",
        link: "/prodi/README.md",
      },
      {
        text: "Solusi Program Studi Statistika Meningkatkan Akademik Mahasiswa (SIGMA)",
        icon: "calculator",
        link: "/sigma/README.md",
      },
      {
        text: "Temu Sapa Online Mahasiswa Statistika On-Going",
        icon: "hand",
        link: "/temusapa/README.md",
      },
      {
        text: "Tutorial Webinar (TUWEB) Statistika",
        icon: "chalkboard-user",
        link: "/tuweb/README.md",
      },
      {
        text: "Webinar Series Statistika",
        icon: "chalkboard-user",
        link: "/webinarseries/prodi/README.md",
      },
    ]
  };
  